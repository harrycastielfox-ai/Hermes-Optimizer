...
function PanelIconView({ icon, className }: { icon: PanelIcon; className?: string }) {
  if (typeof icon === "string") {
    return (
      <img src={icon} alt="" aria-hidden="true" className={className ?? "w-5 h-5 object-contain"} />
    );
  }
...
export function InfoRow({
  ...
  return (
    <div className="grid grid-cols-[38px_minmax(96px,0.45fr)_minmax(0,1fr)] items-center gap-3">
      ...
      <span className="min-w-0 break-words text-[13px] leading-snug text-foreground/90">
        {value}
      </span>
    </div>
  );
