...
function ConfiguracoesPage() {
  ...
  function exportReport() {
    ...
    try {
      const payload = {
        ...
        source: "Hermes Optimizer - Centro de Seguranca e Recuperacao",
        localOnly: true,
        status,
        snapshots,
        events,
  ...
  function prepareHistoryClear() {
    ...
    setNotice("Confirmacao forte recebida. Nenhum historico foi apagado nesta fase.");
  }
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <main className="flex-1 px-5 pt-6 pb-4 overflow-auto xl:px-8 xl:pt-7">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-bold tracking-[0.22em] text-primary mb-2">CONFIGURACOES</p>
              <h1 className="text-[clamp(26px,2vw,32px)] leading-tight font-bold tracking-tight text-foreground">Configuracoes</h1>
              <p className="text-[13px] text-muted-foreground mt-1">Preferencias locais e seguranca operacional do Hermes.</p>
            </div>
            <button
              type="button"
          ...
          <section className="rounded-2xl border border-border/60 bg-card p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_34px_-20px_rgba(15,23,42,0.16)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="text-lg font-bold text-foreground">Seguranca e Recuperacao</h2>
                  <p className="mt-1 max-w-3xl text-sm leading-relaxed text-muted-foreground">
                    Restauracao, snapshots, logs locais, historico de alteracoes e relatorio operacional em um unico lugar.
                  </p>
                </div>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <StatusCard
                icon={LockKeyhole}
                label="ESTADO"
                value={status?.readyForRealActions ? "Pronto" : "Atencao"}
                sub={status?.storage ?? "AppData local do Hermes"}
                tone={status?.readyForRealActions ? "success" : "warning"}
              />
            </div>
